...
struct button {
  ...
  void drawToScale(sf::RenderTexture& surface, sf::RenderWindow& window, sf::Text& sftext, float x, float y) {
    if (isHovering) {
    ...
    swoosh::game::drawToScale(surface, window, sprite);
    ...
    swoosh::game::drawToScale(surface, window, sftext);
  }
};
